...
Stroke subtype classification was derived from the primary ICD code assigned at discharge. Ischemic stroke was defined by ICD-10 codes I63.x or ICD-9 codes 433.x--434.x; intracerebral hemorrhage by I61.x; subarachnoid hemorrhage by I60.x; transient ischemic attack by G45.x or 435.x; and all remaining cerebrovascular codes were grouped as other cerebrovascular disease. The final analytic cohort comprised [N] unique patients.

### 2.2.1 ICD Code Limitations

Stroke diagnosis and subtype classification relied on ICD-coded discharge diagnoses, which are subject to well-documented limitations in sensitivity and positive predictive value (PPV). McCormick et al. (2015) reported that the PPV of ICD codes for stroke identification ranges from 56% to 97% across studies, with hemorrhagic subtypes generally achieving higher coding accuracy than ischemic events [8b]. The transition from ICD-9 to ICD-10 coding (October 2015 in the United States) introduces a temporal discontinuity within the MIMIC-IV observation period (2008--2022), as coding practices and granularity changed between systems. Our ICD-9 code ranges (433--436) do not include codes 430 (subarachnoid hemorrhage), 431 (intracerebral hemorrhage), or 432 (other intracranial hemorrhage), meaning that hemorrhagic stroke patients coded exclusively under ICD-9 in the pre-2015 period may be underrepresented. Similarly, ICD-10 code I62 (other and unspecified nontraumatic intracranial hemorrhage) was not included in the original extraction criteria. Because MIMIC-IV v3.1 is predominantly composed of post-2015 admissions coded under ICD-10, the impact of these ICD-9 gaps on the overall cohort composition is expected to be modest but is acknowledged as a limitation.

The variable PPV of ICD coding has implications for the Bayesian Network structure learning: edges learned from potentially misclassified subtypes (e.g., ischemic stroke misclassified as "other cerebrovascular") may not fully reflect the true conditional dependency structure. This limitation is inherent to all retrospective studies using administrative coding data and should be considered when interpreting the learned network edges, particularly those involving stroke subtype as a parent or child node.
...
Median imputation was selected as the primary approach because (a) missing data rates were modest (0--22.4% across variables), (b) median is robust to the right-skewed distributions characteristic of laboratory values (e.g., creatinine, INR), and (c) it avoids the distributional assumptions required by model-based methods. We acknowledge that median imputation assumes data are missing completely at random (MCAR), an assumption that is likely violated for laboratory values in the ICU setting, where the decision to order a test is clinically driven (informative missingness). The binary missingness indicators partially mitigate this concern by allowing downstream models to learn associations between missingness patterns and outcomes. As a sensitivity analysis, the pipeline supports mean imputation as an alternative; comparison of key evaluation metrics under both imputation strategies is recommended to assess robustness. More sophisticated approaches such as multiple imputation by chained equations (MICE) or MissForest were not employed in this iteration but are acknowledged as potential improvements for future work.
...
Critically, plausibility evaluation was performed on data inverse-transformed to the original clinical scale (using the stored normalization parameters) rather than on the normalized [--1, 1] representation. This ensures that physiological range checks are applied against meaningful clinical units (e.g., age in years, blood pressure in mmHg) and avoids spurious violations arising from evaluation on normalized values.
...
## 2.10 Associational Scenario Simulation

A secondary application of the hybrid generative framework is the capacity to perform associational scenario exploration at the individual patient level. Scenario simulations were conducted by modifying one or more attributes of a given patient's static profile (e.g., introducing atrial fibrillation, increasing age by a decade, adding chronic kidney disease in combination with diabetes mellitus) and regenerating the corresponding 72-hour ICU time-series trajectories conditioned on the modified profile.

For each scenario, 20 to 30 synthetic time-series samples were generated to characterize the distribution of plausible trajectories under the modified clinical context. Baseline trajectories (conditioned on the original, unmodified profile) were generated in parallel using the same number of samples. Comparisons between baseline and modified trajectory distributions were expressed as the difference in mean trajectories with accompanying standard deviations, enabling visual and quantitative assessment of the expected physiological differences associated with the simulated clinical modification.

**Important methodological caveat.** These scenario simulations propagate changes through the Bayesian Network's learned conditional distributions, which encode observational associations from the MIMIC-IV cohort rather than experimentally identified causal mechanisms. Consequently, the resulting trajectory differences reflect correlational patterns and should not be interpreted as causal treatment effects or individual-level counterfactual outcomes in the formal potential outcomes framework. Unmeasured confounders, collider bias, and selection effects inherent in the observational data may distort the apparent associations. The simulations are best understood as structured hypothesis-generating explorations of how patient-level characteristics covary with expected ICU physiological trajectories, analogous to subgroup-stratified descriptive analyses rather than randomized intervention comparisons.
...
All analyses were performed in Python 3.12. Key software dependencies included DuckDB 1.1 for data extraction, pgmpy 1.0.0 for Bayesian Network construction and sampling, PyTorch 2.4 for DoppelGANger implementation and training, SDV 1.34 for CTGAN and TVAE baselines, and scikit-learn 1.5 for evaluation classifiers.

**Code and Data Availability.** All source code, configuration files, SQL extraction queries, and Jupyter notebooks for reproducing the analyses described in this manuscript are publicly available at https://github.com/matheus-rech/MIMIC-Ext-Stroke under the MIT License. Access to the underlying MIMIC-IV data requires a separate credentialed data use agreement through PhysioNet (https://physionet.org/content/mimiciv/).
