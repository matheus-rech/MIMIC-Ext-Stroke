...
def encode_categoricals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cat_cols = [
        "gender", "race", "insurance", "stroke_subtype",
...
def split_data(
    ...
    stratify_col = (
        "hospital_expire_flag" if "hospital_expire_flag" in df.columns else None
    )
...
def preprocess_pipeline(config: dict) -> dict:
    ...
    drop_cols = [
        "icd_code", "icd_version", "seq_num", "icd_title",
        "last_careunit", "intime", "outtime", "dod",
    ...
    all_cols = sorted(
        ...
        | set(val_encoded.columns)
        | set(test_encoded.columns)
    )
    for col in all_cols:
    ...
    numeric_cols = [
        ...
        and c not in exclude_from_norm
    ]
    ...
    ts_processed.to_parquet(
        out_path / "timeseries_processed.parquet", index=False
    )
